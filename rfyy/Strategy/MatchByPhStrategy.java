 ...
 **/
public class MatchByPhStrategy implements MatchStrategy {

    @Override
    public StrategyEnum getType() {
        return StrategyEnum.PH;
    }
    ...
    public boolean match(XsfMatchData xsf, MatchContext ctx) {
        ...
        while (fpIterator.hasNext()) {
            ...
            Set<String> matchedPhs = fp.getPhs();
            // 根据spmc和ph找到对应的采购单
            ...
            if (!CollectionUtils.isEmpty(matchedCgds)) {
                // 先比较合计
                BigDecimal cgdJeTotal = CalcUtils.calcCgdJe(matchedCgds);
                int cgdSlTotal = CalcUtils.calcCgdSl(matchedCgds);
                if (CalcUtils.compareJe(fp.getJshjBd(), cgdJeTotal)
                    ...
                    for (Map.Entry<String, List<Cgd>> entry : matchedRqCgds.entrySet()) {
                        ...
                        if (!matchedRqs.isEmpty()) {
                            String rq = DateMatchUtils.findClosestRq(matchedRqs, fp.getKprq());
